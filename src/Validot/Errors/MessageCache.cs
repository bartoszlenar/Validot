#pragma warning disable CA1854 // Prefer the 'IDictionary.TryGetValue(TKey, out TValue)' method // todo: refactor!
...
{
    private readonly Dictionary<string, Dictionary<int, IReadOnlyList<string>>> _messages = new();

    private readonly Dictionary<int, int> _messagesAmount = new();

    private readonly Dictionary<string, Dictionary<string, Dictionary<int, IReadOnlyList<string>>>> _messagesWithPathArgs = new();

    private readonly Dictionary<string, Dictionary<int, IReadOnlyDictionary<int, IReadOnlyList<ArgPlaceholder>>>> _placeholders = new();
    ...
    {
        var allErrorsIds = _messages.Count != 0
            ? _messages.FirstOrDefault().Value.Keys.ToArray() // todo: First() ?
            : Array.Empty<int>();
